//
//  Admin1ViewController.swift
//  SCIM
//
//  Created by yannian liu on 16/2/16.
//  Copyright © 2016年 yannian liu. All rights reserved.
//

import UIKit

class Admin1ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    var SCIMdateAndID_Array : [String] = []
    
    @IBOutlet weak var QOLHistory_TableView: UITableView!
    var QOLdateAndID_Array : [String] = []

//    var SCIMBottomConstraint = NSLayoutConstraint()
    ...
    @IBOutlet weak var UserIDSearch_TextField: UITextField!
    ...
    @IBAction func Search_ButtonAction(_ sender: AnyObject) {
        ...
        UserIDSearch_String = self.UserIDSearch_TextField.text!
        if (UserIDSearch_String == ""){
           UserIDSearch_String = "All"
        }
    ...
    @IBAction func SCIMGetMore_ButtonAction(_ sender: AnyObject) {
    ...
    @IBAction func QOLGetMore_ButtonAction(_ sender: AnyObject) {
    ...
    override func viewDidLoad() {
        ...
        UserIDSearch_TextField.layer.borderColor = UIColor(red:60.0/255.0, green:175.0/255.0, blue:240.0/255.0, alpha:1).cgColor as CGColor
        UserIDSearch_TextField.layer.borderWidth = 1.0
        UserIDSearch_TextField.layer.cornerRadius = 0.1 * From_Button.bounds.size.width
        UserIDSearch_TextField.clipsToBounds = true
        UserIDSearch_TextField.backgroundColor = UIColor.white
        ...
        SCIMRefreshControl.addTarget(self, action: #selector(Admin1ViewController.SCIMPullToFresh), for: UIControlEvents.valueChanged)
        QOLRefreshControl.addTarget(self, action: #selector(Admin1ViewController.QOLPullToFresh), for: UIControlEvents.valueChanged)
...
//        self.myScrollView.frame = self.view.bounds
//
//        self.myScrollView.contentSize.height = self.SCIMHistory_TableView.frame.height + 200 + 600
//        
//        print(self.SCIMHistory_TableView.frame.height)
//       print( self.SCIMHistory_TableView.frame.size.height)
//        print (self.SCIMHistory_TableView.contentSize.height)
//        print ( self.myScrollView.contentSize.height)
//        print ( self.myScrollView.frame.height)
//        print(self.SCIMHistory_TableView.rowHeight)
//
//        
//        //self.myScrollView.contentSize.height = self.SCIMHistory_TableView.frame.height
//
  ...
  //  }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if tableView == SCIMHistory_TableView{
            return self.SCIMdateAndID_Array.count
        }else {
            return self.QOLdateAndID_Array.count
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView == SCIMHistory_TableView{
            ...
            cell.textLabel!.text = String(self.SCIMdateAndID_Array[indexPath.row])
            cell.accessoryType = UITableViewCellAccessoryType.disclosureIndicator
            setCellStyle(cell)
            setTabelView(tableView)
           // tableView.frame.size.height = CGFloat (Double(SCIMdateAndID_Array.count) * 41.00);
            return cell
            
        }else{
            let cell:UITableViewCell = self.QOLHistory_TableView.dequeueReusableCell(withIdentifier: "cell2")! as     UITableViewCell
            cell.textLabel!.text = String(self.QOLdateAndID_Array[indexPath.row])
            cell.accessoryType = UITableViewCellAccessoryType.disclosureIndicator
            setCellStyle(cell)
            setTabelView(tableView)
            //tableView.frame.size.height = CGFloat (Double(QOLdateAndID_Array.count) * 41.00);
            return cell
        }
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if tableView == SCIMHistory_TableView {
            ...
            //}
            performSegue(withIdentifier: "ShowOneHistorySCIMAdmin_Segue", sender: self)
            ...
            //}
            performSegue(withIdentifier: "ShowOneHistoryQOLAdmin_Segue", sender: self)
    ...
    func getSCIMHistory (_ urlString: String, userIDInput_p: String, fromDate_p: String, toDate_p: String){
        ...
        self.Time1 = 5
        self.Timer1 = Timer.scheduledTimer(timeInterval: 1.0, target: self, selector: #selector(Admin1ViewController.TimeOut1), userInfo: nil, repeats: true)
        ...
        var SCIMTempDateAndTime_Array : [String] = []
        var SCIMTempUserId_Array : [String] = []
        ...
        let task = URLSession.shared.dataTask(with: request as URLRequest, completionHandler: {data, response, error in
            ...
            do {
                self.SCIMJSON = try JSONSerialization.jsonObject(with: data! as Data, options:.mutableLeaves) as! NSDictionary
                //print (self.SCIMJSON)
                var lengthOfJSON = 0;
                if let SCIMHistory = self.SCIMJSON["SCIMHistory"] as? [[String: AnyObject]] {
                    self.SCIMAllHistory_Array = self.SCIMAllHistory_Array+SCIMHistory
                    //print("+++\(self.SCIMHistory_Array.description)")
                    for SCIMHistory in SCIMHistory {
                        if let oneDateAndTime = SCIMHistory["dateAndTime"] as? String {
                            SCIMTempDateAndTime_Array.append(oneDateAndTime)
                        }
                        if let oneUserID = SCIMHistory["userID"] as? String {
                            SCIMTempUserId_Array.append(oneUserID)
                        }
                        lengthOfJSON = lengthOfJSON+1
                    }
                }
                for i in 0...SCIMTempDateAndTime_Array.count {
                //for var i = 0; i < SCIMTempDateAndTime_Array.count; i++ {
                    self.SCIMdateAndID_Array.append("USER ID: \(SCIMTempUserId_Array[i])  TIME:\(SCIMTempDateAndTime_Array[i])" )
                }
                ...
                DispatchQueue.main.async(execute: {
                    ...
                    self.SCIMHistoryTableViewHeight_Constraint.constant = CGFloat(self.SCIMdateAndID_Array.count-9) * 44
                    if self.SCIMdateAndID_Array.count < 9{
                        self.SCIM_ScrollView.contentSize.height = CGFloat(9 * 44 + 60)
                    }else{
                        self.SCIM_ScrollView.contentSize.height = CGFloat((self.SCIMdateAndID_Array.count) * 44 + 60)
                    }
    ...
    func getQOLHistory (_ urlString: String, userIDInput_p: String, fromDate_p: String, toDate_p: String){
        ...
        self.Time2 = 5
        self.Timer2 = Timer.scheduledTimer(timeInterval: 1.0, target: self, selector: #selector(Admin1ViewController.TimeOut2), userInfo: nil, repeats: true)
        ...
        var QOLTempDateAndTime_Array : [String] = []
        var QOLTempUserId_Array : [String] = []
        ...
        let task = URLSession.shared.dataTask(with: request as URLRequest, completionHandler: {data, response, error in
            ...
            do {
                self.QOLJSON = try JSONSerialization.jsonObject(with: data!, options:.mutableLeaves) as! NSDictionary
                //print (self.QOLJSON)
                var lengthOfJSON = 0;
                if let QOLHistory = self.QOLJSON["QOLHistory"] as? [[String: AnyObject]] {
                    ...
                    for QOLHistory in QOLHistory {
                        if let oneDateAndTime = QOLHistory["dateAndTime"] as? String {
                            QOLTempDateAndTime_Array.append(oneDateAndTime)
                        }
                        if let oneUserID = QOLHistory["userID"] as? String {
                            QOLTempUserId_Array.append(oneUserID)
                        }
                        lengthOfJSON = lengthOfJSON+1
                ...
                for index in 0...QOLTempDateAndTime_Array.count {
                //for var i = 0; i < QOLTempDateAndTime_Array.count; i++ {
                    self.QOLdateAndID_Array.append("USER ID: \(QOLTempUserId_Array[index])  TIME:\(QOLTempDateAndTime_Array[index])" )
                }
                ...
                DispatchQueue.main.async(execute: {
                    ...
                    self.myActivityIndicatorView.isHidden = true
                    self.QOLRefreshControl.endRefreshing()
                    self.QOLHistoryTableViewHeight_Constraint.constant = CGFloat(self.QOLdateAndID_Array.count-9) * 44
                    if self.QOLdateAndID_Array.count < 9{
                        self.QOL_ScrollView.contentSize.height = CGFloat(9 * 44 + 60)
                    }else{
                        self.QOL_ScrollView.contentSize.height = CGFloat((self.QOLdateAndID_Array.count) * 44 + 60)
                    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if(segue.identifier == "ShowOneHistorySCIMAdmin_Segue") {
